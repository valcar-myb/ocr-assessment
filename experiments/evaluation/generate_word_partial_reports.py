"""
Script to generate word accuracy partial reports using ocreval 'wordacc' command
Phase 1 (Word): Generate individual word accuracy reports for each file
"""
...
def main():
    ...
    # Generate word accuracy partial reports for each dataset
        ...
        for tool_dir in text_outputs_base_dir.iterdir():
            if tool_dir.is_dir():
                ...
                try:
                    ...
                    partial_reports = evaluator.generate_word_partial_reports(
                    ...
                    print(f"[✓] Generated {len(partial_reports)} word accuracy partial reports")
                    
                except Exception as e:
                    print(f"[✗] Error processing {ocr_tool_name}: {e}")
    
    print(f"\n{'='*80}")
    print("Word accuracy partial reports generation complete!")
    print(f"{'='*80}")
    print(f"\nPartial reports saved to: {evaluator.partials_word_base_dir}")
    print(f"\nNext step: Run 'python aggregate_word_reports.py' to create aggregated word reports")
